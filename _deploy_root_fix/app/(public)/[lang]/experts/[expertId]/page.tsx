...
export async function generateMetadata({
  ...
  params: { expertId: string; lang: string }
}): Promise<Metadata> {
  const experts = getAllExperts()
  ...
  const currentUrl = `${baseUrl}/${routeLang}/experts/${expert.id}`
  const title = `${expert.name} | SIA Council of Five`
  const description = expert.bio[contentLang] || expert.bio.en
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates(`/experts/${expert.id}`, { baseUrl }),
    },
...
export default function ExpertProfilePage({
  ...
  const routeLang = normalizePublicRouteLocale(params.lang)
  const contentLang = toDictionaryLocale(routeLang)
  const tr = getT(contentLang)
  const bio = expert.bio[contentLang] || expert.bio.en
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://siaintel.com'
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Terminal', item: `${baseUrl}/${routeLang}` },
      {
        ...
        position: 2,
        name: 'Council',
        item: `${baseUrl}/${routeLang}/experts`,
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: expert.name,
        item: `${baseUrl}/${routeLang}/experts/${expert.id}`,
      },
    ],
  }
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 space-y-8 relative overflow-hidden backdrop-blur-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-6 bg-black/40 border border-white/5 rounded-3xl text-center">
                  <span className="block text-[9px] font-black text-white/20 uppercase tracking-widest mb-2">
                    EXPERIENCE
                  </span>
            ...
            <div className="p-10 rounded-[3rem] bg-gradient-to-br from-blue-600/10 to-transparent border border-blue-500/20 space-y-6">
              ...
              <p className="text-sm text-slate-400 font-light leading-relaxed italic">
                Member of the Council of Five. Subject Matter Expert for {expert.category}{' '}
          ...
          <div className="lg:col-span-8 space-y-12">
              ...
              <div className="flex items-center gap-6 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] mt-1">
                    Verified Background & Meta-Analysis
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-6">
                    {expert.education.map((edu, i) => (
                      <div
                        key={i}
                        className="relative pl-6 border-l border-white/10 hover:border-blue-500 transition-colors group/item"
                      >
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-6">
                    {expert.experience.map((exp, i) => (
                      <div
                        key={i}
                        className="relative pl-6 border-l border-white/10 hover:border-emerald-500 transition-colors group/item"
                      >
              ...
              <div className="p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 space-y-8 backdrop-blur-xl group">
                ...
                <div className="flex flex-wrap gap-2">
                  {expert.expertise.map((skill, i) => (
                    <span
                      key={i}
                      className="px-4 py-2 bg-white/5 border border-white/5 rounded-full text-[9px] font-black text-white/40 uppercase tracking-widest hover:text-white hover:border-white/20 transition-all cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
              ...
              <div className="p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 space-y-8 backdrop-blur-xl group">
                ...
                <div className="space-y-3">
                  {expert.certifications.map((cert, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 p-3 bg-black/40 border border-white/5 rounded-2xl"
                    >
            ...
            <div className="p-8 text-center border-t border-white/5 pt-12">
              <p className="text-[9px] text-white/20 font-black uppercase tracking-[0.4em] mb-4">
                Identity_Verification_Protocol: SIA_EEAT_V2
              </p>
