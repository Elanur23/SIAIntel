...
const OG_IMAGE = 'https://images.unsplash.com/photo-1639762681485-074b7f938ba0?q=80&w=1200&h=630&fit=crop'
...
const categorySeo: Record<string, { title: string; desc: string; keywords: string[] }> = {
  en: {
    ...
    keywords: ['bitcoin analysis', 'crypto intelligence', 'ethereum signals', 'DeFi tracking', 'whale alert', 'blockchain data', 'crypto institutional', 'altcoin analysis', 'on-chain data', 'crypto market', 'BTC signals', 'ETH analysis']
  },
  tr: {
    ...
    keywords: ['bitcoin analizi', 'kripto istihbarat', 'ethereum sinyalleri', 'DeFi takibi', 'balina alarmı', 'blockchain verisi', 'altcoin analizi', 'on-chain veri', 'kripto piyasası', 'BTC sinyalleri']
  },
  de: {
    ...
    keywords: ['Bitcoin Analyse', 'Krypto Intelligenz', 'Ethereum Signale', 'DeFi Tracking', 'Wal Alert', 'Blockchain Daten', 'Altcoin Analyse', 'On-Chain Daten', 'Krypto Markt', 'BTC Signale']
  },
  fr: {
    ...
    keywords: ['analyse bitcoin', 'intelligence crypto', 'signaux ethereum', 'DeFi suivi', 'alerte baleine', 'données blockchain', 'analyse altcoin', 'données on-chain', 'marché crypto', 'signaux BTC']
  },
  es: {
    ...
    keywords: ['análisis bitcoin', 'inteligencia cripto', 'señales ethereum', 'DeFi seguimiento', 'alerta ballena', 'datos blockchain', 'análisis altcoin', 'datos on-chain', 'mercado cripto', 'señales BTC']
  },
  ru: {
    ...
    keywords: ['анализ биткоина', 'крипто аналитика', 'сигналы ethereum', 'DeFi отслеживание', 'Whale Alert', 'данные блокчейна', 'анализ альткоинов', 'on-chain данные', 'рынок криптовалют']
  },
  ar: {
    ...
    keywords: ['تحليل البيتكوين', 'استخبارات التشفير', 'إشارات إيثريوم', 'تتبع DeFi', 'تنبيه الحيتان', 'بيانات blockchain', 'تحليل العملات البديلة', 'بيانات on-chain']
  },
  jp: {
    title: '暗号資産インテリジェンス | ビットコイン・イーサリアム・DeFi分析',
    desc: '機関投資家向けのリアルタイム暗号資産インテリジェンス、クジラ追跡、on-chainシグナル。',
    keywords: ['ビットコイン分析', '暗号資産インテリジェンス', 'イーサリアムシグナル', 'DeFi追跡', 'クジラアラート', 'ブロックチェーンデータ', 'アルトコイン分析', 'オンチェーンデータ']
  },
  zh: {
    title: '加密货币情报 | 比特币、以太坊和DeFi分析',
    desc: '面向机构投资者的实时加密货币情报、鲸鱼追踪、DeFi分析和链上信号。',
    keywords: ['比特币分析', '加密货币情报', '以太坊信号', 'DeFi追踪', '鲸鱼预警', '区块链数据', '山寨币分析', '链上数据', '加密货币市场', 'BTC信号']
  },
}
...
export async function generateMetadata({ params }: { params: { lang?: string } }): Promise<Metadata> {
  ...
  const currentUrl = `${baseUrl}/${lang}/crypto`
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'x-default': `${baseUrl}/en/crypto`,
        en: `${baseUrl}/en/crypto`, tr: `${baseUrl}/tr/crypto`,
        de: `${baseUrl}/de/crypto`, fr: `${baseUrl}/fr/crypto`,
        es: `${baseUrl}/es/crypto`, ru: `${baseUrl}/ru/crypto`,
        ar: `${baseUrl}/ar/crypto`, ja: `${baseUrl}/jp/crypto`,
...
export default function CryptoLayout({ children, params }: { children: React.ReactNode; params: { lang: string } }) {
  const lang = params?.lang || 'en'
  const t = categorySeo[lang] || categorySeo.en
  const pageUrl = `${baseUrl}/${lang}/crypto`
