...
export default function SearchPage({ params }: { params: { lang: string } }) {
  ...
  const labels: Record<string, { title: string; noResults: string; placeholder: string; back: string; scanning: string }> = {
    en: { title: 'Search_Results', noResults: 'No Intelligence Found', placeholder: 'Awaiting Query...', back: 'Back to Terminal', scanning: 'Scanning Global Nodes...' },
    tr: { title: 'Arama_Sonuçları', noResults: 'İstihbarat Bulunamadı', placeholder: 'Sorgu Bekleniyor...', back: 'Terminale Dön', scanning: 'Küresel Düğümler Taranıyor...' },
  }

  const l = labels[params.lang] || labels.en
      ...
      <div className="container mx-auto px-4 lg:px-10 py-12 md:py-20 relative z-10">
          ...
          <div className="space-y-6">
            ...
            {query && (
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">Query_ID:</span>
        ...
        <AnimatePresence>
          {isLoading && (
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-400 animate-pulse">{l.scanning}</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {!query && !isLoading && (
          <div className="flex flex-col items-center justify-center py-40 text-center space-y-8 bg-white/[0.01] border border-white/5 rounded-[4rem] backdrop-blur-sm">
            ...
            <p className="text-white/30 text-lg font-black uppercase italic tracking-[0.2em]">{l.placeholder}</p>
          </div>
        )}
        ...
        {query && !isLoading && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-40 text-center space-y-10 bg-white/[0.01] border border-white/5 rounded-[4rem] backdrop-blur-sm">
            ...
            <div className="space-y-4">
              <h2 className="text-3xl font-black uppercase italic tracking-tighter text-white">{l.noResults}</h2>
              <p className="text-slate-500 text-sm font-medium uppercase tracking-widest italic">SIA_SENTINEL could not locate any matching intelligence for "{query}"</p>
            </div>
            <Link
              href={`/${params.lang}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {results.map((item: any, idx: number) => (
            <Link key={item.id || idx} href={`/${params.lang}/news/${item.slug || item.id}`}>
              <motion.div
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6 relative z-10">
                  ...
                  <span className="text-[9px] text-white/20 font-mono tracking-widest uppercase">{item.time || 'LIVE'}</span>
                </div>
