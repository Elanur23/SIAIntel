...
const OG_IMAGE = 'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?q=80&w=1200&h=630&fit=crop'
...
const categorySeo: Record<string, { title: string; desc: string; keywords: string[] }> = {
  en: {
    ...
    keywords: ['stock market analysis', 'nasdaq intelligence', 'S&P500 signals', 'equity flow', 'dark pool data', 'wall street intelligence', 'institutional trading', 'NYSE analysis', 'equity analysis', 'stock signals', 'market intelligence', 'stock market today']
  },
  tr: {
    ...
    keywords: ['borsa analizi', 'nasdaq istihbarat', 'S&P500 sinyalleri', 'hisse senedi analizi', 'dark pool verileri', 'Wall Street', 'kurumsal işlemler', 'NYSE analizi', 'hisse sinyalleri', 'borsa bugün']
  },
  de: {
    ...
    keywords: ['Aktienmarkt Analyse', 'Nasdaq Intelligenz', 'S&P500 Signale', 'Aktienfluss', 'Dark-Pool-Daten', 'Wall Street', 'institutioneller Handel', 'NYSE Analyse', 'Aktiensignale', 'Marktintelligenz']
  },
  fr: {
    ...
    keywords: ['analyse boursière', 'intelligence Nasdaq', 'signaux S&P500', 'trading institutionnel', 'données dark pool', 'Wall Street', 'NYSE analyse', 'signaux actions', 'bourse aujourd\'hui']
  },
  es: {
    ...
    keywords: ['análisis bursátil', 'inteligencia Nasdaq', 'señales S&P500', 'trading institucional', 'datos dark pool', 'Wall Street', 'análisis NYSE', 'señales acciones', 'bolsa hoy']
  },
  ru: {
    ...
    keywords: ['анализ акций', 'Nasdaq аналитика', 'S&P500 сигналы', 'институциональная торговля', 'данные dark pool', 'Уолл-стрит', 'NYSE анализ', 'фондовый рынок сегодня']
  },
  ar: {
    ...
    keywords: ['تحليل الأسهم', 'استخبارات ناسداك', 'إشارات S&P500', 'تداول مؤسسي', 'بيانات dark pool', 'وول ستريت', 'تحليل NYSE', 'إشارات الأسهم']
  },
  jp: {
    title: '株式市場インテリジェンス | ナスダック・S&P500・株式分析',
    desc: '機関投資家向けの株式市場シグナル、株式フロー分析、ダークプールデータ、ウォール街インテリジェンス。',
    keywords: ['株式市場分析', 'ナスダックインテリジェンス', 'S&P500シグナル', '機関投資家取引', 'ダークプールデータ', 'ウォール街', 'NYSE分析', '株式シグナル', '株式市場今日']
  },
  zh: {
    title: '股市情报 | 纳斯达克、标普500和股票分析',
    desc: '面向机构的股市信号、股票流分析、暗池数据和华尔街情报。实时纳斯达克、标普500信号。',
    keywords: ['股市分析', '纳斯达克情报', '标普500信号', '机构交易', '暗池数据', '华尔街', 'NYSE分析', '股票信号', '股市今日', '权益分析']
  },
}
...
export async function generateMetadata({ params }: { params: { lang?: string } }): Promise<Metadata> {
  ...
  const currentUrl = `${baseUrl}/${lang}/stocks`
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'x-default': `${baseUrl}/en/stocks`,
        en: `${baseUrl}/en/stocks`, tr: `${baseUrl}/tr/stocks`,
        de: `${baseUrl}/de/stocks`, fr: `${baseUrl}/fr/stocks`,
        es: `${baseUrl}/es/stocks`, ru: `${baseUrl}/ru/stocks`,
        ar: `${baseUrl}/ar/stocks`, ja: `${baseUrl}/jp/stocks`,
        zh: `${baseUrl}/zh/stocks`,
      },
    },
    openGraph: {
...
export default function StocksLayout({ children, params }: { children: React.ReactNode; params: { lang: string } }) {
  const lang = params?.lang || 'en'
  const t = categorySeo[lang] || categorySeo.en
  const pageUrl = `${baseUrl}/${lang}/stocks`
