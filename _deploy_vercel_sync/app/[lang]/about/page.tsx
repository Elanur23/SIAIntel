...
const aboutSeo: Record<string, { title: string; description: string; keywords: string[] }> = {
  en: {
    ...
    description: 'Autonomous financial intelligence powered by advanced AI and the Council of Five elite analysts.',
  ...
  tr: {
    ...
    description: 'Gelişmiş yapay zeka ve Beşler Konseyi seçkin analistleri tarafından desteklenen otonom finansal istihbarat.',
...
export async function generateMetadata({ params }: { params: { lang?: string } }): Promise<Metadata> {
  ...
  const t = getRecordValue(aboutSeo, lang, 'en')
...
export default function AboutPage({ params }: { params: { lang: string } }) {
  const lang = params.lang || 'en'
  const content = getRecordValue(ABOUT_CONTENT, lang, 'en')
      ...
      <div className="container mx-auto px-4 lg:px-10 py-12 md:py-20 relative z-10">
        ...
        <header className="mb-28 max-w-5xl">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div className="text-white/20 text-[9px] font-mono tracking-widest uppercase">Verified_Node_Status</div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-32">
          {content.sections.map((section, index) => (
            <div key={index} className="p-10 lg:p-16 rounded-[4rem] bg-white/[0.02] border border-white/5 space-y-8 relative overflow-hidden backdrop-blur-xl group">
        ...
        <section className="mb-32">
          <div className="flex items-center gap-6 mb-16">
             ...
             <div>
                <h2 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-none">Council_of_Five</h2>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-2">Elite human analysts validating the neural-core signals</p>
             </div>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {experts.map((expert) => (
              <Link key={expert.id} href={`/${lang}/experts/${expert.id}`}>
                <div className="p-8 rounded-[3rem] bg-[#0A0A0C] border border-white/10 hover:border-blue-500/30 transition-all shadow-2xl flex flex-col h-full group">
                  <div className="mb-6 flex items-center justify-between">
                    <div>
                       ...
                       <p className="text-emerald-500 font-black text-[9px] uppercase tracking-[0.2em] mt-1">{expert.title}</p>
                    </div>
        ...
        {content.regulatoryReferences && content.regulatoryReferences.length > 0 && (
          <section className="bg-gradient-to-br from-blue-600/5 to-transparent rounded-[4rem] p-12 md:p-24 border border-blue-500/20 relative overflow-hidden group shadow-2xl">
            ...
            <div className="flex items-center gap-6 mb-16 relative z-10">
               ...
               <h2 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-none">Global_Framework</h2>
            </div>

            <div className="grid gap-10 relative z-10">
              {content.regulatoryReferences.map((ref, index) => (
                <div key={index} className="p-8 bg-black/40 border border-white/5 rounded-[2.5rem] hover:bg-black/60 transition-all">
                  <a 
                    href={ref.url}
                    target="_blank"
